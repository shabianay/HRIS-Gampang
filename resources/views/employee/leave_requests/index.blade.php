        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-3 text-left">Tipe Cuti</th>
                        <th class="text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-3 text-left">Tanggal</th>
                        <th class="text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-3 text-left">Durasi</th>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                    @forelse($leaveRequests as $lr)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{{ $lr->leaveType->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ \Carbon\Carbon::parse($lr->start_date)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($lr->end_date)->format('d/m/Y') }}</td>
