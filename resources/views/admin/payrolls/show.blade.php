<x-admin-layout>
    <div>
        <div class="mb-6 flex items-center justify-between">
            <a href="{{ route('payrolls.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-primary-600 hover:text-primary-700">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                Kembali
            </a>
            @if($payroll->status != 'paid')
                <div class="flex items-center gap-2">
                    <a href="{{ route('payrolls.edit', $payroll) }}" class="btn-secondary text-sm">
            ...
            @endif
            <a href="{{ route('payrolls.print', $payroll) }}" class="btn-secondary text-sm">
        ...
        <div class="card p-6 sm:p-8">
            <div class="text-center mb-8 pb-6 border-b border-slate-200">
                ...
                <p class="text-sm text-slate-500 mt-1">PT. HRIS GAMPANG</p>
                <p class="text-sm text-slate-500">Periode: {{ $payroll->period }}</p>
            </div>
