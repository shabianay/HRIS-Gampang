<x-admin-layout>
    <div>
        <div class="flex items-center mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Komponen Gaji</h1>
                <p class="text-sm text-slate-500 mt-0.5">{{ $salaryComponent->name }}</p>
            </div>
        </div>
            ...
            <form action="{{ route('salary-components.update', $salaryComponent) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="space-y-1.5">
                        ...
                        <x-text-input id="name" name="name" type="text" class="block w-full" :value="old('name', $salaryComponent->name)" />
                        <x-input-error :messages="$errors->get('name')" class="mt-1" />
                    </div>
                    <div class="space-y-1.5">
                        <x-input-label for="code" value="Kode" />
                        <x-text-input id="code" name="code" type="text" class="block w-full" :value="old('code', $salaryComponent->code)" />
                    ...
                    <div class="space-y-1.5">
                        ...
                        <x-text-input id="amount" name="amount" type="number" class="block w-full" :value="old('amount', $salaryComponent->amount)" />
                        <x-input-error :messages="$errors->get('amount')" class="mt-1" />
                    </div>
                    <div class="space-y-1.5">
                        <x-input-label for="calculation" value="Perhitungan" />
                        <select id="calculation" name="calculation" class="input-field">
                            <option value="fixed" {{ old('calculation', $salaryComponent->calculation) == 'fixed' ? 'selected' : '' }}>Fixed</option>
                            <option value="percentage" {{ old('calculation', $salaryComponent->calculation) == 'percentage' ? 'selected' : '' }}>Persentase</option>
                        </select>
                        <x-input-error :messages="$errors->get('calculation')" class="mt-1" />
                    </div>
                    <div class="md:col-span-2 space-y-1.5">
                        <x-input-label for="description" value="Deskripsi" />
                        <textarea id="description" name="description" rows="3" class="input-field">{{ old('description', $salaryComponent->description) }}</textarea>
