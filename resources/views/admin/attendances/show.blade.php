<x-admin-layout>
    <div>
        <div class="mb-6">
            <a href="{{ route('attendances.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-primary-600 hover:text-primary-700">
        ...
        <div class="card p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pegawai</p>
                    <p class="text-sm font-medium text-slate-900 mt-0.5">{{ $attendance->employee->full_name }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">NIK</p>
                    <p class="text-sm font-medium text-slate-900 mt-0.5">{{ $attendance->employee->nik }}</p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Departemen</p>
                    <p class="text-sm font-medium text-slate-900 mt-0.5">{{ $attendance->employee->department?->name ?? '-' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    @if($attendance->latitude && $attendance->longitude)
                        ...
                        <a href="https://www.google.com/maps?q={{ $attendance->latitude }},{{ $attendance->longitude }}" target="_blank" class="text-xs text-primary-600 hover:text-primary-700 mt-1 inline-flex items-center gap-1">
            ...
            <div class="mt-6 pt-6 border-t border-slate-200">
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Perangkat / Browser</p>
                <p class="text-sm text-slate-700 bg-slate-50 p-3 rounded-xl border border-slate-200 break-words font-mono text-xs">{{ $attendance->device_info ?? 'Tidak tersedia' }}</p>
            </div>
