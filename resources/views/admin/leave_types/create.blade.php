<x-admin-layout>
    <div>
        <div class="flex items-center mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Tambah Tipe Cuti</h1>
                <p class="text-sm text-slate-500 mt-0.5">Tambah jenis cuti baru</p>
            </div>
        </div>
            ...
            <form action="{{ route('leave-types.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <x-text-input id="quota" name="quota" type="number" class="block w-full" :value="old('quota')" />
                        <x-input-error :messages="$errors->get('quota')" class="mt-1" />
                    </div>
                    <div class="space-y-1.5 flex items-end pb-3">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input id="is_paid" name="is_paid" type="checkbox" value="1" {{ old('is_paid') ? 'checked' : '' }} class="sr-only peer">
